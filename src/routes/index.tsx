...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Life XP turns your goals into an arcade quest. Plan with AI, earn XP, unlock badges, and watch friends level up." },
      { property: "og:title", content: "Life XP — Level Up Your Goals" },
      { property: "og:description", content: "Turn daily, weekly, monthly and quarterly goals into XP. Plan with AI. Earn badges. Beat the high score." },
    ...
    <div className="min-h-screen scanlines crt-flicker relative overflow-hidden">
      ...
      <header className="relative z-10 px-6 py-5 flex items-center justify-between border-b border-border/60 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          ...
          <span className="font-pixel text-sm sm:text-base text-neon-green text-glow-green">LIFE XP</span>
        </div>
        <nav className="hidden md:flex items-center gap-8 font-pixel text-[10px] uppercase tracking-widest text-muted-foreground">
          <a href="#how" className="hover:text-neon-cyan transition">How it works</a>
          <a href="#badges" className="hover:text-neon-magenta transition">Badges</a>
      ...
      <section className="relative z-10 px-6 pt-20 pb-28 max-w-6xl mx-auto text-center">
        ...
        <p className="mt-8 max-w-2xl mx-auto text-lg sm:text-xl text-muted-foreground">
          Life XP turns your goals into an arcade quest. Plan with an AI co-pilot,
          smash daily missions, earn XP, and unlock pixel badges as you ascend the leaderboard.
        </p>
        ...
        <div className="mt-16 max-w-md mx-auto bg-card/80 backdrop-blur p-6 pixel-border">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="text-left">
              <div className="font-pixel text-xs text-neon-magenta text-glow-magenta">CMDR_NOVA</div>
      ...
      <section id="how" className="relative z-10 px-6 py-20 max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[
            { color: "green" as const, title: "AI CO-PILOT", body: "Chat with your AI strategist to break dreams into daily, weekly, monthly and quarterly missions." },
            { color: "magenta" as const, title: "EARN XP", body: "Every task is worth XP. Daily quests fuel streaks. Weekly bosses drop bigger rewards." },
            { color: "cyan" as const, title: "UNLOCK BADGES", body: "Pixel-perfect badges and new ranks unlock as you level. Show them off on your feed." },
          ].map((c) => (
            <div key={c.title} className="bg-card/70 backdrop-blur p-6 pixel-border hover:-translate-y-1 transition-transform duration-300">
              <Invader color={c.color} size={5} className="invader-float mb-4" />
              <h3 className={`text-sm mb-3 ${c.color === "green" ? "text-neon-green text-glow-green" : c.color === "magenta" ? "text-neon-magenta text-glow-magenta" : "text-neon-cyan text-glow-cyan"}`}>
                {c.title}
              </h3>
      ...
      <section id="badges" className="relative z-10 px-6 py-20 max-w-6xl mx-auto">
        ...
        <p className="text-center text-muted-foreground mb-12">From Rookie Pilot to Galaxy Commander.</p>
      ...
      <section id="feed" className="relative z-10 px-6 py-20 max-w-4xl mx-auto">
        ...
        <div className="space-y-4">
          {[
            { user: "CMDR_NOVA", color: "magenta" as const, msg: "cleared WEEKLY BOSS — Ship 3 client deliverables", xp: 250 },
            { user: "PIXEL_SAGE", color: "cyan" as const, msg: "unlocked badge: 30-Day Reading Streak", xp: 500 },
            { user: "GALAXY_GWEN", color: "green" as const, msg: "completed quarterly quest — Launch side project", xp: 2000 },
          ].map((p, i) => (
            <div key={i} className="flex items-center gap-4 bg-card/70 p-4 pixel-border">
              <Invader color={p.color} size={4} />
              <div className="flex-1">
                <span className={`font-pixel text-[10px] ${p.color === "magenta" ? "text-neon-magenta" : p.color === "cyan" ? "text-neon-cyan" : "text-neon-green"}`}>
                  {p.user}
                </span>{" "}
      ...
      <section id="feedback" className="relative z-10 px-6 py-24 max-w-3xl mx-auto text-center">
        <h2 className="text-2xl sm:text-3xl text-neon-yellow text-glow-cyan mb-6" style={{ textShadow: "0 0 12px var(--neon-yellow)" }}>
          GAME FEEDBACK
        </h2>
        <p className="text-muted-foreground mb-8">
          Found a bug? Got an idea for a new boss fight? Players who submit feedback earn rare XP boosters.
        </p>
